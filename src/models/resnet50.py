...
Original file is located at
    https://colab.research.google.com/drive/1F825OHFxjH-UAug-kcAkkNdY205rU2NH
"""
...
class block(nn.Module):
    ...
    def forward(self, x):
        ...
        out += torch.nn.functional.interpolate(identity, size=(24, 28))
        out = self.relu(out)
...
# layers: The number of times we need to use the BasicBlock
    def __init__(self, block, layers, img_channels , num_classes):
        ...
        self.layer4 = self._make_layer(block, layers[3], out_channels = 512, stride = 2) #Now here the output will be 512*4=2048
    ...
    def forward(self, x):
        ...
        out = self.layer1(out) # Calls make_layer which in turn calls block multiple times
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
...
# This is the make_layer function which makes a layers according to our requirements.
    def _make_layer(self, block, num_residual_blocks, out_channels , stride = 1):
        ...
        for _ in range(num_residual_blocks - 1):
            layers.append(block(self.in_channels, out_channels)) #in_channels is 256->64, 64*4(256) again and stride=1
